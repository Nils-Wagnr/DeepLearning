...
OUT = Path(__file__).with_name("ClaimGuard_Capstone_Presentation.pptx")
NOTES_OUT = Path(__file__).with_name("ClaimGuard_Presentation_Notes.md")
...
def build_deck() -> Presentation:
    ...
    # 4 — Architecture
    s = new_slide(prs, 4, "Pipeline architecture", "Nils", "1:10", "System architecture")
    stages = [
    ...
    # 6 — Demo
    s = new_slide(prs, 6, "Demo on our real RQ5 report", "Nick", "1:30", "Demo")
    ...
    cmd = "python run_claimguard.py `\n  --input Reports/NW2_RQ5_Report.pdf --verifier openai `\n  --output outputs/rq5_openai.json --markdown-output outputs/rq5_openai.md"
    ...
    metric_card(s, 8.25, 1.52, 3.92, "1.000", "IEEE parser accuracy", GREEN, "5-field-complete test cases")
    metric_card(s, 8.25, 3.08, 3.92, "0.267", "Citation-needed F1", RED, "50 manually labeled RQ sentences")
    metric_card(s, 8.25, 4.64, 3.92, "0.353", "SciFact validation Macro-F1", AMBER, "450 validation pairs")
    ...
    # 8 — Model comparison
    s = new_slide(prs, 8, "Model comparison: speed, control, and taxonomy", "Nils", "1:15", "Local vs. frontier")
    ...
    rows = [
        ("Heuristic", "0.900 F1*", "~0 ms", "Transparent baseline", "Relies on surface patterns", BLUE),
        ("LoRA", "0.172 F1*", "9.3 ms", "Cheap local inference", "SciFact has 3 vs. 5 labels", AMBER),
        ("Ollama", "supported†", "40.9 s", "Private local review", "Slow; one diagnostic claim", GREEN),
        ("OpenAI", "insufficient†", "1.89 s", "Structured frontier review", "Paid/external; one diagnostic claim", PURPLE),
    ]
    ...
    textbox(s, 0.98, 6.12, 11.35, 0.3, "* 30-case gold benchmark     † same real RQ5 claim; diagnostic only — not an accuracy comparison", size=12.5, color="92400E", bold=True, align=PP_ALIGN.CENTER)
    ...
    # 9 — Works / limits
    s = new_slide(prs, 9, "What works — and where it breaks", "Nils", "0:55", "Limits")
    ...
    add_bullets(s, 1.02, 2.35, 4.95, 3.25, [
        ...
        "Offline fallback and real CUDA LoRA path",
    ...
    add_bullets(s, 7.18, 2.35, 4.95, 3.25, [
        ...
        "Binoculars' calibrated 14B pair exceeds 8 GB VRAM",
    ], size=16, color=INK, gap=16)
    ...
    # 10 — Conclusion
    s = new_slide(prs, 10, "Takeaways", "Nick + Nils", "0:35", "Conclusion")
    set_fill(s.background, NAVY)
    ...
    textbox(s, 0.82, 5.55, 11.5, 0.45, "Next: expand double-annotated real claims and run the full OpenAI/Ollama gold benchmark.", size=17, color=WHITE, bold=True, align=PP_ALIGN.CENTER)
...
def validate(prs: Presentation) -> None:
    if len(prs.slides) != 11:
        raise RuntimeError(f"Expected 11 slides, got {len(prs.slides)}")
    for index, slide in enumerate(prs.slides, start=1):
