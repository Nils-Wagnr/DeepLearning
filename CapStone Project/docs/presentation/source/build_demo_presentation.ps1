...
$presentationRoot = [System.IO.Path]::GetFullPath((Join-Path $source ".."))
$buildDirectory = Join-Path $source "build"
$assetsDirectory = Join-Path $presentationRoot "assets"
$sourceDeck = Join-Path $buildDirectory "ClaimGuard_Capstone_Praesentation_DE.pptx"
$outputDeck = Join-Path $presentationRoot "ClaimGuard_Capstone_Praesentation_DE_mit_UI_Demo.pptx"
$outputPdf = Join-Path $presentationRoot "ClaimGuard_Capstone_Praesentation_DE_mit_UI_Demo.pdf"
$videoSource = Join-Path $buildDirectory "ClaimGuard_UI_Demo_Source.pptx"
$video = Join-Path $assetsDirectory "ClaimGuard_UI_Demo.mp4"
$framesDirectory = Join-Path $assetsDirectory "demo_frames"

$expectedSource = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot "."))
if ($source -ne $expectedSource) {
    throw "Refusing to use an unexpected source directory: $source"
}
...
foreach ($generated in @($outputDeck, $outputPdf, $videoSource, $video)) {
    ...
    if ($resolvedParent -notin @($presentationRoot, $buildDirectory, $assetsDirectory)) {
...
try {
    ...
    $demoSlide = $targetDeck.Slides.Item(7)
