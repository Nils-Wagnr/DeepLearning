...
versus cross-encoder retrieval, citation-needed detection, and Binoculars. State explicitly that
Binoculars is an authorship-risk signal with known domain/language failure modes, not proof of AI
use. Add at least five primary references in the final bibliography.
...
4. Module 3 chunks evidence and retrieves it lexically or with normalized
   ...
   schema.
5. Optional modules provide SciFact LoRA training and Binoculars integration. The LoRA script
   records model, seed, device, automatically detected attention targets, effective batch size,
   sample counts, and evaluation metrics.
...
| IEEE reference parsing | 5 | 1.000 | - | all-fields accuracy 1.000 |
...
A one-claim backend diagnostic produced different outputs: OpenAI `gpt-5.4-mini` returned
`insufficient_evidence` in 1.889 s using 506 tokens; local `qwen3:1.7b` returned `supported` in
40.902 s using 388 prompt and 447 generated tokens. These are execution diagnostics, not accuracy
metrics. Full 30-case Ollama/OpenAI evaluation remains required for a fair frontier/local leaderboard.
...
threshold calibration. The SciFact result is a single-seed teaching experiment rather than a
tuned state-of-the-art result. DeBERTa and DistilBERT trials showed substantial sensitivity to
learning rate and randomly initialized classification/pooling heads.
